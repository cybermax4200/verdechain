...
export function LifecycleTimeline({ events }: LifecycleTimelineProps) {
  ...
  return (
    <div className="relative">
      <div className="absolute left-[23px] top-0 bottom-0 w-0.5 bg-gray-200 dark:bg-gray-800" />
      <div className="space-y-6">
        {sortedEvents.map((event, index) => {
          ...
          const emissionsBarWidth = event.emissionsKg ? (event.emissionsKg / maxEmissions) * 100 : 0;

          return (
            <div key={eventId} className="relative pl-14">
              <div
                className="absolute left-4 top-1 w-8 h-8 rounded-full flex items-center justify-center text-sm border-2 border-white dark:border-gray-950 shadow-sm"
                        ...
                        <div className="space-y-1">
                          {event.materialInputs.map((input, i) => (
                            <div key={i} className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                              ...
                              <span className="font-medium">{input.amount} {input.unit}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {event.participants.map((p, i) => (
                            <span
                              key={i}
                              className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs text-gray-700 dark:text-gray-300"
                            >
                              {p.name} ({p.role})
                            </span>
                          ))}
